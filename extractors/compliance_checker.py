...
class ComplianceChecker(BaseExtractor):
    ...
    llm: SerializeAsAny[LLM] = Field(
        default_factory=lambda: Ollama(model="llama3.2:1b", base_url="http://localhost:11434"),
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        super().__init__(
            llm=llm or Ollama(model="llama3.2:1b", base_url="http://localhost:11434"),
            issues=issues,
    ...
    async def _aextract_issues_from_node(self, node: BaseNode) -> Dict[str, str]:
        """Extract issues from a node and return its metadata dict."""
